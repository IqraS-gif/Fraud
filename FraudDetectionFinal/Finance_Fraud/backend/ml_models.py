...
class HybridFraudModel:
    ...
    def generate_ai_reasoning(self, tx_dict, p_fraud, recon_error, risk_level, language='en', history_context=None):
        """Uses Groq to generate reasoning and historical trend analysis."""
        try:
            history_str = ""
            if history_context:
                history_str = "\n".join([
                    f"- Amt: ₹{tx['amount']}, Loc: {tx['location']}, Status: {tx['status']}" 
                    for tx in history_context
                ])

            prompt = f"""
            As a Financial Fraud Expert, analyze this transaction and compare it with the user's historical patterns.
            ...
            HISTORICAL PATTERNS (Last 5):
            {history_str if history_str else "No prior history available."}
            
            ANALYTICS:
            - Risk Level: {risk_level}
            - Fraud Probability: {p_fraud*100:.1f}%
            - Anomaly Score: {recon_error:.4f}
            
            TASK:
            1. Provide "REASONING": 2-3 bullet points on why THIS transaction is risky.
            2. Provide "TRENDS": 2-3 bullet points matching this to past transactions or global fraud trends (e.g., "Velocity spike observed", "Common pattern for account takeover").
            ...
            # Split into Reasoning and Trends
            reasoning = "Analysis based on hybrid matching."
            trends = "Monitoring historical deviations."
            
            if "REASONING:" in raw_output and "TRENDS:" in raw_output:
                parts = raw_output.split("TRENDS:")
                reasoning = parts[0].replace("REASONING:", "").strip()
                trends = parts[1].strip()
            
            return reasoning, trends

        except Exception as e:
            print(f"Groq Error: {e}")
            return "Analysis based on hybrid matching.", "Monitoring historical deviations."

    def predict(self, tx_dict, user_id, language='en', history_context=None):
        ...
        # 4. Groq Reasoning & Trends
        reasoning, trends = self.generate_ai_reasoning(tx_dict, p_fraud, recon_error, risk_level, language, history_context)
